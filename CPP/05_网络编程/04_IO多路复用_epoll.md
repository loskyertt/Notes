...
2. **就绪链表**
    ...
    - **作用**：当某个设备（如网卡）收到数据时，内核通过中断处理程序，直接通过回调函数将对应的 FD 添加到这个链表中。
...
> [!warning] `epfd` 本质上也是一个文件，用完必须 `close(epfd)`，否则会造成 fd 泄漏。
...
> 为了实现这种长期保存，内核必须在自己的受管内存里开辟空间，把你的配置**拷贝**进去。
