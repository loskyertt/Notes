...
`socket()` 用于在操作系统内核中创建一个新的套接字，并返回该套接字的**文件描述符**。该套接字可被用于后续的网络通信操作，如绑定地址（`bind`）、监听连接（`listen`）、发送与接收数据等。
...
- **参数**:
    ...
    - `protocol`: 具体协议编号，通常设为 `0`，表示由系统根据 `domain` 和 `type` 自动选择
	    - `IPPROTO_TCP`: 流式传输（`SOCK_STREAM`）默认使用的是面向连接的 TCP；
		- `IPPROTO_UDP`: 报式传输（`SOCK_DGRAM`）默认使用的是无连接的 UDP。
...
`bind` 的作用是**把一个“地址”（IP + 端口）绑定到创建的 socket 上，告诉操作系统“以后有数据发到这个 IP 和端口，都交给这个 socket 处理”**。该操作通常在调用 `listen()` 之前完成，是 TCP 服务器建立监听前的必要步骤。
...
- **参数**:
    ...
    - `addr`: 指向 [struct sockaddr](00_网络基础知识.md) 类型的指针，包含要绑定的地址信息（如 IP 地址和端口号）。
...
- **参数**:
    ...
    - `addrlen`: **传入传出**参数，指向 `socklen_t` 的指针
	    - **传入时**：告诉内核你传入的 addr 缓冲区有多大（防止内核写越界）；
...
> `__THROW` 是 glibc 定义的一个**宏**，C++ 模式下展开成 `noexcept`，用于告诉编译器“这个函数保证不抛 C++ 异常”。
...
int main() {  
  // 1. 创建 socket（:: 表示调用全局命名空间下的 socket 函数）  
  ...
  const uint16_t port  = 8080;  // 在 TCP/IP 协议栈中，端口号占用 16 位（2 字节）  
  ...
  while (true) {  
    ...
    printf("accept success! new connection, conn_fd=%d,\n", conn_fd);  
  
    // 5. 接收客户端的数据：假设一次 recv 能读完所有数据  
    while (true) {  
      char buf[1024]    = {0};  
      const ssize_t len = ::recv(conn_fd, buf, sizeof(buf), 0);  
      if (len < 0) {  
      ...
      printf("recv: conn_fd=%d, msg=%s, len=%ld\n", conn_fd, buf, len);  
  
      // 6. 向客户端发送数据  
      ::send(conn_fd, buf, static_cast<size_t>(len), 0);  
    }  
  }  
...
int main() {  
  ...
  while (true) {  
    ...
    if (::send(client_fd, message, sizeof(message), 0) < 0) {  
      printf("send error: errno = %d, errmsg = %s\n", errno, strerror(errno));  
      return 1;  
    }  
    printf("send success!\n");  
    ...
    // 4. 接受服务端的数据  
    char buf[1024]    = {0};  
    const ssize_t len = ::recv(client_fd, buf, sizeof(buf), 0);  
    if (len < 0) {  
      printf("recv error: errno = %d, errmsg = %s\n", errno, strerror(errno));  
      return 1;  
    }  
    printf("echo: msg = %s, len=%ld\n", buf, len);  
  }  
...
> [!important]
> `send(...)` 并不是直接把数据发到网线上，它的实际工作是：**把数据从你的进程拷贝到内核的发送缓冲区**，真正发送到对端的事情由内核在后台做。
...
这时候 `send()` 不会等缓冲区腾出空间，而是直接**返回已写入的字节数**。
...
> [!important]
> 同样，`recv(...)` 也不是“直接从网线上读数据”，而是“**从内核 socket 接收缓冲区（recv buffer）拷贝数据到用户态 buffer 中**”：
...
> [!question] 不过这里有一个比 `send` 更棘手的问题：**你怎么知道 `total` 是多少？**
...
| 创建时机   | `socket()` 成功返回时     | `accept()` 成功返回时        |
...
> [!example]
> 若有 100 个客户端同时连接，服务端进程中就会存在 1 个监听 Socket 和 100 个已连接 Socket，共 101 个 fd。
...
`accept()` 本身**不参与三次握手**，握手由内核协议栈在后台全程处理。`accept()` 只是从 **Accept 队列**中取出（dequeue）一个已建立的 `struct sock`，为其创建 `struct socket` 和 `struct file`，并返回新的 fd（即 `conn_fd`）。若 Accept 队列为空，`accept()` 默认阻塞等待。

> [!Caution]
> 读写缓冲区（`sk_rcvbuf` / `sk_sndbuf`）在三次握手完成时就已随 `struct sock` 一并分配，**并非** `accept()` 之后才创建。`accept()` 只是将这个已就绪的 socket 交接给用户态。
...
虽然 `{目的 IP, 目的端口, 协议}` 对所有连接相同，但同一时刻每个客户端自身的 `{源 IP, 源端口}` 组合必然唯一。数据包到达网卡时，内核以五元组为 key 查找对应的 `struct tcp_sock` 控制块，再经由 `struct socket` → `struct file` → `fd_array[]` 的反向映射，将数据送达应用层对应的 `conn_fd`。
