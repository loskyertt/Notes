...
> [!note]
> 下面的代码要启用 C++ 23 或以上，才能使用 `std::println`，否则请使用 `printf("port 8080 in network order: 0x%04X\n", port_net);`。
...
Linux 的 Socket API 设计于 C 语言早期，为了兼容性，使用了一个通用的 `struct sockaddr` 作为所有地址结构的“**最小公共接口视图**”。实际编程中，我们使用具体的 `sockaddr_in` (IPv4) 或 `sockaddr_in6` (IPv6)。

![img](imgs/00_网络基础知识/02.png)

从上图可以看出 `sockaddr` 和 `sockaddr_in6` 内存布局并不完全一样大，`sockaddr` 只有 16 字节，而 `sockaddr_in6` 是 28 字节。下面逐个看下结构体的大小：

```cpp
struct sockaddr {
    unsigned short sa_family;  // 2 字节
...
struct sockaddr_in {
    ...
    char sin_zero[8];          // 8 字节 (填充)
};                             // 总共 16 字节 —— 和 sockaddr 完全一样！
```

```cpp
struct sockaddr_in6 {
    unsigned short sin6_family;   // 2 字节
    unsigned short sin6_port;     // 2 字节
    unsigned long sin6_flowinfo;  // 4 字节
    struct in6_addr sin6_addr;    // 16 字节 (IPv6 地址，比 IPv4 的 4 字节大得多)
    unsigned long sin6_scope_id;  // 4 字节
};                                // 总共 28 字节
```

**关键原因**：IPv6 地址本身就是 128 位（16 字节），比 IPv4 地址（32 位，4 字节）大得多，`sockaddr_in6` 为了装下这个更大的地址，结构体整体也必须更大。而 `sockaddr` 当初设计时（IPv6 还不存在的年代），`sa_data` 只留了 14 字节，根本装不下 IPv6 需要的数据。

因此 `sockaddr` 描述的其实是**接口层面的兼容**，而不是“内存布局完全相同”。准确来说，`sockaddr` 和各种具体地址结构体之间的关系是：

**它们的开头部分（前几个字节）布局一致，都是从“地址族字段”开始**，这才是能强转、能通过一个通用指针传递的真正原因：

```cpp
struct sockaddr {
    unsigned short sa_family;  // 开头 2 字节：地址族
    char sa_data[14];
};

struct sockaddr_in {
    short sin_family;          // 开头 2 字节：地址族，位置和 sa_family 对齐
    // ... 后面是IPv4专属数据
};

struct sockaddr_in6 {
    unsigned short sin6_family; // 开头 2 字节：地址族，位置同样对齐
    // ... 后面是 IPv6 专属数据（更多、更大）
};
```

只要**前2个字节（地址族字段）位置对齐**，函数就能先读这 2 个字节判断“这到底是 IPv4 还是 IPv6”，再决定要不要把指针强转成更具体的类型来读取剩余字段：

```cpp
void printAddr(struct sockaddr* addr) {
    if (addr->sa_family == AF_INET) {
        struct sockaddr_in* addr4 = (struct sockaddr_in*)addr; // 强转读取 IPv4 专属字段
        // ...
    } else if (addr->sa_family == AF_INET6) {
        struct sockaddr_in6* addr6 = (struct sockaddr_in6*)addr; // 强转读取 IPv6 专属字段（超出 sockaddr 原本 16 字节的范围）
    }
}
```

**关键点**：当处理 IPv6 时，程序实际操作的内存块**远远大于 `sockaddr` 的16字节**——只是这块更大的内存，最初的16字节恰好和 `sockaddr` 的字段布局对得上，`sockaddr` 只是用来“框住”这块更大内存的**开头一部分**，而不是整个内存块都长这样。

> [!question] 那实际调用时，内存到底是怎么分配的？

这是最容易搞混的地方。实际写代码时，调用者会先声明一个**具体协议对应的、真实大小的结构体**：

```cpp
struct sockaddr_in6 addr6;  // 编译器实际分配了 28 字节的内存
addr6.sin6_family = AF_INET6;
// ...填充IPv6专属字段...

bind(sockfd, (struct sockaddr*)&addr6, sizeof(addr6)); 
//            ^^^^^^^^^^^^^^^^^^^^^^^  ^^^^^^^^^^^^^
//            只是把指针类型“伪装”成sockaddr*，       但传的 sizeof 还是真实的 28 字节！
//            指向的内存依然是 28 字节大小
```

> [!important]
> `bind()` 函数内部拿到的是 `sockaddr*` 类型的指针（只是类型标签），但它**不会真的只读 16 字节**，而是会先看 `sa_family` 字段判断这是 IPv4 还是 IPv6，再根据传入的 `addrlen` 参数（这里是 `sizeof(addr6)` = 28）知道“这块内存实际有多大”，然后在内部把指针再强转回 `sockaddr_in6*`，去访问真正的 28 字节数据。

现代写法其实更推荐用 `sockaddr_storage`，因为它内部特意设计成**足够大、能装下任何已知协议族的地址**（通常 128 字节），从根本上避免了"某个具体类型比 `sockaddr` 大"导致的越界问题：

```cpp
struct sockaddr_storage {
    unsigned short ss_family;
    char __ss_padding[...]; // 足够大的填充，保证能装下sockaddr_in6等任何已知类型
    // ...
};
```
...
|结构体|大小|关系说明|
|---|---|---|
|`sockaddr`|16字节|通用"视图"，只保证前2字节（地址族）位置对齐，不代表实际内存大小|
|`sockaddr_in`|16字节|恰好和 `sockaddr` 一样大（历史设计巧合，IPv4地址小）|
|`sockaddr_in6`|28字节|比 `sockaddr` 大得多，因为IPv6地址本身就更大|
|`sockaddr_storage`|通常128字节|专门设计得足够大，能安全容纳任何协议族的地址，是更现代、更安全的通用容器|
...
> `p` 表示 presentation（人可读的点分形式），`n` 表示 numeric（网络序二进制）
