...
要注意 **声明引用**时的 `&` 和**取地址符** `&` 的区别：

- **取地址符 (Address-of Operator)**：出现在 **表达式右侧** 或 **单独使用**。如 `ptr = &e;`，表示取 `e` 的地址。
    
- **引用声明 (Reference Declaration)**：出现在 **类型名称之后**（如 `T&`）。它不代表地址，而是代表**别名**。
...
- **常量指针 (`const Type *`)**：指向的内容不能变，但指针可以指向别处。这显然不是引用的行为。
...
- **指针常量 (`Type *const`)**：指针本身的值（保存的地址）不能变，但指向的内容可以变。

```cpp
int* const p = &a;   // p 不可改指向（p 不能变），*p 可改
*p = 3;              // OK
```

> [!tip]
> 这完美契合了引用“初始化后不可重绑定，但可以修改目标值”的特性。在大多数编译器（如 GCC, Clang, MSVC）的底层实现中，引用底层就是 `Type *const`。
...
`void*` 是 C/C++ 中的**无类型指针**（通用指针），它可以指向**任意类型**的数据，但编译器**不知道它指向的具体类型**。
...
```c
// malloc 返回 void*，调用方自行转型
int* arr = (int*)malloc(10 * sizeof(int));

// memcpy / memset 接受 void*，可操作任意类型的内存
memcpy(dst, src, n);
...
```c
// C 库常见写法：用 void* 隐藏实现细节
void* ctx = lib_create_context();
lib_do_something(ctx);
lib_destroy_context(ctx);
```
...
// 调用方传入自己的数据，回调时取回
MyState state = { ... };
register_callback(my_cb, &state);
```

> [!question] `void*` 这么强大，那么推荐使用 `void*` 吗？
> **在纯 C 中**：没有替代品，合理使用无可避免。
> **在 C++ 中** ：几乎总是应该避免。原因如下：

|问题|说明|
|---|---|
|🔴 类型安全丧失|编译器无法检查转型是否正确，错误转型在运行时才暴露|
|🔴 可维护性差|读代码时无法知道指针实际指向什么|
|🔴 UB 风险高|错误的类型转换导致未定义行为|
|🔴 无法触发析构|持有 `void*` 时对象析构函数不会自动调用|
...
void my_sort(std::vector<T>& v) {
    std::sort(v.begin(), v.end()); // 类型安全，且更快
}
```
...
std::any val = 42;
val = std::string("hello");  // 可随时换类型

// 取值时类型不对会抛异常，而非 UB
int x = std::any_cast<int>(val);  // ✅
```

#### `std::variant`（C++17）—— 类型受限的联合体
...
#### `std::function` —— 替代回调中的 `void* user_data`

```cpp
// 旧 C 风格
void register_cb(void (*cb)(void*), void* data);

// 现代 C++ 风格：用 lambda 捕获上下文，无需 void*
void register_cb(std::function<void()> cb);

MyState state;
register_cb([&state]() { use(state); });
```
