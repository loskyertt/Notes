...
1. **多申请 `alignof(Slot) - 1` 字节**：给后续手动对齐留余量。
2. **申请原始内存**：`::operator new(raw_size)` 只分配字节，不构造 `T`。
3. **挂入 Block 链表**：确保池析构时能找到每个大块。
4. **对齐 Slot 起始地址**：`base` 必须满足 `alignof(Slot)`。
5. **切分 Slot 并头插 FreeList**：每个 Slot 作为一个空闲节点。

#### 为什么要手动对齐 `base`

`::operator new` 返回的整块内存起始地址通常满足最大基础对齐，但 `Block` 头部之后的位置不一定仍然满足 `alignof(Slot)`：
...
> 一个大小 `size`，至少要补到多少字节，才满足对齐？

对象池里的函数回答的是：

> 一个地址 `p`，至少要向后移动到哪里，才满足对齐？

因为指针本身不能直接做位运算，所以第一步要把指针转成整数地址：

```cpp
auto addr = reinterpret_cast<std::uintptr_t>(p);
```

`std::uintptr_t` 是一种无符号整数类型，设计目标是能够完整保存一个指针地址。转成整数后，才能使用 `&`、`~` 这类位运算。
...
1. **`addr + alignment - 1`**：先把地址向后推一点，使未对齐地址越过下一个对齐边界。
2. **`~(alignment - 1)`**：构造一个低位清零的掩码。
3. **`&` 掩码**：清掉低位，得到最近的合法对齐地址。

以 `alignment = 8` 为例：

```text
alignment      = 8      = 0b1000
alignment - 1  = 7      = 0b0111
~(alignment-1) = ~7     = ...1111_1000
```

`& ~7` 会清掉最低 3 位，因此结果一定是 8 的整数倍。
...
```cpp
align_forward(reinterpret_cast<char*>(0x1003), 8)
```
...
> [!tip]
> 记忆方式：`align_upward` 是“把大小补齐”，`align_forward` 是“把地址推齐”。公式一样，只是前者操作数字大小，后者操作指针地址转换后的整数值。

#### 为什么这里比内存池版本更严谨

在内存池版本中，通常只把 `chunk_size_` 对齐：

```cpp
chunk_size_ = align_upward(chunk_size, alignof(std::max_align_t));
```

这保证了每个 Chunk 的大小是对齐倍数，但还隐含依赖一个条件：

```text
Block 头部之后的 base 地址本身也要对齐
```
...
得到的 `base` 必须刚好满足目标对齐。主流平台上 `sizeof(Block)` 往往是指针大小，很多时候刚好没问题；但从更严谨的角度看，**跳过一个 Header 后的位置不一定仍然满足 `Slot` 的对齐要求**。
...
所以 `raw` 不能直接当作 `Slot*`。必须通过：

```cpp
char* base = align_forward(raw, alignof(Slot));
```

把它推到下一个 32 字节对齐地址，例如：

```text
base = 0x1020
```

这也是 `raw_size` 要额外加上 `alignof(Slot) - 1` 的原因：
...
因为 `align_forward()` 最多可能向后移动 `alignment - 1` 字节。如果不额外预留这部分空间，对齐后可能导致最后一个 Slot 越过整块 Block 的尾部。

> [!warning]
> 对齐错误属于 C++ 中非常隐蔽的未定义行为。它在 x86 上可能只是性能下降，在某些平台上可能直接崩溃。
