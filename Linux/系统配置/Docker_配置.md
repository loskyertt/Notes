...
> [!warning] 必须指定 conda-forge
> `-c conda-forge` 一定要加，默认的 `conda.org` 源没有 `docker`。

安装好的 `docker` 和 `docker-compose` 在该路径下：
...
> [!warning] 根据实际情况填写代理端口。

3. 如果自己建了私有镜像仓库，需要 docker 绕过代理服务器直连，那么配置 `NO_PROXY` 变量：
...
> [!note] 相关笔记
> 终端代理配置参见 [[05_Zsh_配置#1. 终端代理设置]]，Steam 代理配置参见 [[Steam_配置]]。

## 1.4 命令简化配置

> [!tip] 推荐加到 `.bashrc` 或者 `.zshrc` 中。
