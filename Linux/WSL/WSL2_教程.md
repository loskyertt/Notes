...
- 首先需要在 **控制面板 -> 程序 -> 启用或关闭Windows功能** 处开启 **Hyper-V** 和 **适用于 Windows 的 Linux 子系统**，然后重启电脑即可。

- 默认有 2 个版本（WSL 和 WSL2），WSL 是使用和主机相同的 IP 地址；WSL2 是更加独立的 Linux 子系统，有单独的 IP 地址，通过 Windows 主机访问互联网，建议安装 WSL2，可以用 `systemd`。如果安装 WSL2 还需要安装一个软件：[wsl_update_x64](https://wslstorestorage.blob.core.windows.net/wslblob/wsl_update_x64.msi),该软件来自于[Windows 官网](https://learn.microsoft.com/zh-cn/windows/wsl/install-manual#step-4---download-the-linux-kernel-update-package)。
...
> [!tip]
> 上面这种命令行方式容易因为网络原因卡住，因此，建议离线下载安装：[microsoft/WSL](https://github.com/microsoft/WSL)，下载好后直接双击安装即可。
...
- 也可以用仓库里线现成的：[Nevuly/WSL2-Linux-Kernel-Rolling](https://github.com/Nevuly/WSL2-Linux-Kernel-Rolling)。
...
%% 以下是本 vault 中与 WSL 相关的笔记 %%
- [[WSL_容器_GUI]] — WSL2 和 Docker 容器中运行 GUI 程序
- [[02_WSL2_NixOS]] — 在 WSL2 中安装 NixOS
...
- [WSL2挂载USB设备](https://blog.csdn.net/qq_59475883/article/details/123314000)