...
这是一段再熟悉不过的 CMakeLists.txt 代码，我们通过 `find_package` 命令寻找到了 OpenCV 库，并获得了其中的导入目标列表。我们可以使用 `message` 验证一下 `OpenCV_LIBS` 包含的内容
...
那么 CMake 是如何通过 `find_package` 命令找到 OpenCV 并且完成配置的呢?
...
- **2. find_package 解析过程** 提到过，Module 模式下需要查找到名为 `Find<PackageName>.cmake` 的配置文件。该文件的查找过程只涉及到两个路径：`CMAKE_MODULE_PATH` 和 CMake 安装路径下的 Modules 目录。即，搜索路径依次为：
...
我们在[【05】目标构建](05_目标构建.md)的导入目标中遇到过类似的情况，但这里的相机 SDK 并没有位于本项目中。
...
CMake 的逻辑是：_“去 `../include` 文件夹里找，看看有没有一个叫 `CameraApi.h` （也可以写 `CameraDefine.h` 或 `CameraStatus.h`）的文件。如果有，就把**包含这个文件的目录路径**（即 `.../include`）存进 `Camera_INCLUDE_DIR` 变量里。”_

如果你指定的 `NAMES` 顺序是 `CameraApi.h CameraDefine.h CameraStatus.h`，而你的目录里这两个文件恰好**不在同一个文件夹**，CMake 会返回**先找到的那个文件**所在的文件夹路径。

> 2. `find_path` 返回的是**目录**，而 `find_library` 返回的是**文件的完整绝对路径**。
...
1. **搜索目标**：它会在你指定的 `PATHS` 下寻找名为 `libMVSDK.lib`（Windows）、`libMVSDK.a`（Linux）、`libMVSDK.so`（Linux）或 `libMVSDK.dll.a`（MinGW）的文件。它会自动补全后缀名。
2. **变量赋值**：如果找到了，`Camera_LIB` 变量的值将是 **文件的完整路径**，例如： `/opt/camera/opt/camera/libMVSDK.so`
3. **注意**：它**不会**只存目录。如果你打印这个变量，你会看到它指向的是那个具体的 `.lib` 文件。
