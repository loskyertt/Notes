...
后，可以解析来自当前目录中 `CMakeLists.txt` 里的内容，并将产物输出到 `build` 目录中。因此，CMake 解析的文件一般都是名为 `CMakeLists.txt` 的文件，当然还有其他类型的文件，在后文会详细介绍。
...
创建好后，在该项目目录下打开终端（Windows 下可以是 PowerShell），输入

```bash
cmake -B build
```

即可在当前文件夹下生成对应的建构档（`build` 目录）。
...
通过上文可以得知，目标具有依赖关系，因此目标不仅只有可执行文件这一形式，还具有别的形式，在[【05】目标构建](05_目标构建.md)会详细给出目标包含的内容。
