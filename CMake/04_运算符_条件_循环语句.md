...
- `TARGET`：如果给定的名称是目标（包括二进制目标、伪目标，可参考[【05】目标构建](05_目标构建.md)），则返回真。格式为：`TARGET xxx`，例如：
...
- `COMMAND`
  ...
  - CMake 直接使用操作系统 API 执行子进程。所有参数都被逐字传递给子进程。没有使用中间shell，所以像 `>` 这样的 shell 操作符被视为普通参数。
