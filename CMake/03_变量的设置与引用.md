...
此类变量具有与 C/C++ 语言中的自动存储期类似的效果，一般情况下，
...
可以看到，`${ADD_MODULE_ARGN}` 在整个条件运算中均未定义，因此他对于这个语句来说是个外部变量。对于外部变量，OpenCV 的做法通常与上面的代码一致，即 `set()` 一个新的变量来记录这个可能会做修改的值，并且在这个变量用完后 `unset()`。
