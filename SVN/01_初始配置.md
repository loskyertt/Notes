...
在 Windows 上安装 SVN，通常分为客户端安装（推荐使用 TortoiseSVN）和服务端搭建（推荐使用 VisualSVN Server）。如果你只是需要下载、上传和管理他人服务器上的代码，只需安装客户端即可。

#### 安装 SVN 客户端（TortoiseSVN）

Windows 上最常用的是 SVN 客户端，它直接集成在 Windows 右键菜单中。

- **下载安装包**：
	* 访问 [TortoiseSVN 官网下载页面](https://tortoisesvn.net/downloads.html)。
	* 根据你的系统选择 64-bit 或 32-bit 下载。
	* 在同页面下方找到 Language packs（语言包），下载 Chinese, simplified（简体中文包）。

- **安装步骤**：
	1. 运行安装包：双击打开下载好的 `TortoiseSVN-...-x64.msi`。
	2. 常规下一步：连续点击 Next。
	3. 开启命令行工具（关键）：
		* 在组件选择页面（Custom Setup），找到 command line client tools。
		* 点击它前面的小图标，选择 Will be installed on local hard drive。
		* 注意：如果你需要在 IntelliJ IDEA、WebStorm 等 IDE 中使用 SVN，必须勾选此项。
	4. 完成安装：点击 Install，完成后点击 Finish。

- **安装中文语言包**：
	* 双击运行下载好的简体中文语言包 .msi 文件。
	* 一路点击下一步直到完成。
	* 切换中文：在桌面空白处点击右键 -> 选择 TortoiseSVN -> Settings -> 在 Language 下拉菜单中选择 中文(简体) -> 点击 应用。 

> [!tip]
> 验证安装是否成功：在电脑任意文件夹内点击鼠标右键，如果菜单中出现了 SVN Checkout (SVN 检出) 和 TortoiseSVN，说明客户端安装成功。
