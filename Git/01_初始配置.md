...
`22`端口如果不行（比如速度很慢）可以改成`443`端口。

### 1.4.2 Git for Windows 配置

1. 用 `Get-Command ssh` 获取 Windows 上的 OpenSSH：

```shell
PS C:\Users\sky> Get-Command ssh
...
2. 让 Git-Bash 用 PowerShell 的 ssh：

```bash
git config --global core.sshCommand "C:/PROGRA~1/OpenSSH/ssh.exe"
```

`PROGRA~1` 就是 `Program Files` 的 DOS 短名，避免空格的影响。

---

# 2. 个人访问令牌

使用 Personal Access Token，PAT 这种方式主要用于临时快速修改（私有）仓库，或者当 SSH 连接不可使用时使用。

1. **生成个人访问令牌：**
   - 登录到你的 GitHub 账户。
   - 进入 [GitHub 个人访问令牌设置页面](https://github.com/settings/tokens)。
   - 点击 `Generate new token` 按钮，选择 `Generate new token(classic)`。
   - 给令牌一个描述性的名称，选择适当的权限范围（至少需要 `repo` 权限，这是访问私有仓库必需的）。
